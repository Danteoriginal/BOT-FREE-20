...
void CBaseGame :: SendFakePlayerInfo( CGamePlayer *player )
{
	...
	IP.push_back( 0 );
	IP.push_back( 0 );
	Send( player, m_Protocol->SEND_W3GS_PLAYERINFO( m_FakePlayerPID, "|c00FF0000OBS", IP, IP ) );
}
...
void CBaseGame :: SendWelcomeMessage( CGamePlayer *player )
{
	...
	{
		SendChat( player, "----------------------------------------------------------------");
		SendChat( player, "[*] Creator: " + m_OwnerName );
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
	{
		(*i)->QueueChatCommand( "/whereis " + joinPlayer->GetName( ) );
	}
...
void CBaseGame :: EventGameStarted( )
{
	...
	if( m_Replay )
	{
		...
		if( m_FakePlayerPID != 255 )
			m_Replay->AddPlayer( m_FakePlayerPID, "|c00FF0000OBS" );
...
void CBaseGame :: CreateFakePlayer( )
{
	...
	if( SID < m_Slots.size( ) )
	{
		...
		IP.push_back( 0 );
		IP.push_back( 0 );
		SendAll( m_Protocol->SEND_W3GS_PLAYERINFO( m_FakePlayerPID, "|c00FF0000OBS", IP, IP ) );
