...
bool CBNET :: Update( void *fd, void *send_fd )
{
	...
	for( vector<PairedAdminCount> :: iterator i = m_PairedAdminCounts.begin( ); i != m_PairedAdminCounts.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			string AdminString = "";
			for( vector<string> :: iterator it = m_Admins.begin( ); it != m_Admins.end( ); ++it ) {
				AdminString += *it + ", ";
			}
			...
			if( Count == 0 )
				...
				QueueChatCommand( m_GHost->m_Language->ThereAreAdmins( m_Server, "Admins (" + UTIL_ToString( Count ) + ") : "+ AdminString.substr( 0, AdminString.length( ) - 2 ) ), i->first, !i->first.empty( ) );
			else {
				QueueChatCommand( m_GHost->m_Language->ThereAreAdmins( m_Server, "Admins (" + UTIL_ToString( Count ) + ") : "+ AdminString.substr( 0, AdminString.length( ) - 2 ) ), i->first, !i->first.empty( ) );
			}
			...
			if( IsAdmin( User ) || IsRootAdmin( User ) )
			{
				...
				******************/
				...
				if( Command == "addadmin" && !Payload.empty( ) ) {
					if( IsRootAdmin( User ) && m_Admins.size( ) < 20 ) {
						if( IsAdmin( Payload ) )
				...
				if( Command == "pub" && !Payload.empty( ) ) {
					...
					m_GHost->m_CurrentGame->CreateFakePlayer( );
				}
